...
class TwoLayerMLP(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    for i in range(scores_shape[0]):
        scores[i] = scores_exp[i]/scores_sum[i]
    ...
    loss = -np.sum(np.diagonal(doted))/loop
    
    reg1 = np.sum(np.diagonal(W1.T @ W1))
    reg2 = np.sum(np.diagonal(W2.T @ W2))
    loss = loss + 0.5*(reg1 + reg2)*reg
    #print('Loss:',loss)
    ...
    db2 = np.sum(dscore, axis=0)/loop   # sum accross rows/samples
    
    
    #print('dscore', dscore)
    # partial derivative of loss wrt. W2    
    dW2 = (dscore.T @ hidden).T/loop + W2*reg

    # hidden layer
    dhidden = np.dot(dscore/loop, np.transpose(W2))
    if self.activation is 'relu':
        dz1 = dhidden
        dz1[z1 <= 0] = 0
    elif self.activation is 'softplus':
